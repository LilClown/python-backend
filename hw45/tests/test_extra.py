...
def test_item_crud_and_filters(client: TestClient) -> None:
    ...
    r = client.get("/item")
    assert r.status_code == HTTPStatus.OK
    listed_ids = [x["id"] for x in r.json()]
    assert i2["id"] not in listed_ids

    r = client.get("/item", params={"show_deleted": True, "limit": 1000})
    listed_ids = [x["id"] for x in r.json()]
    assert {i1["id"], i2["id"], i3["id"]}.issubset(set(listed_ids))

    r = client.get("/item", params={"min_price": 9.99, "limit": 1000})
    data = r.json()
    assert all(x["price"] >= 9.99 for x in data)
    ids = {x["id"] for x in data}
    assert {i1["id"], i3["id"]}.issubset(ids)

    r = client.get("/item", params={"max_price": 10.0, "limit": 1000})
    data = r.json()
    assert all(x["price"] <= 10.0 for x in data)
    ids = {x["id"] for x in data}
    assert i1["id"] in ids  
